...
class JDLAdvertisingView: UIView {
    ...
    private var gifView: JDLGifImageOperation?          // 播放gif视图
    ...
    private func loadDataSource() {
        ...
        let type = JDLGifImageOperation.checkDataType(data: imageData)
        if type == .gif {
            gifView = JDLGifImageOperation.init(frame: adImageView.frame, gifData: imageData)
    ...
    private func getAdData(url: String) -> Data? {
        if isIgnoreCache {  // 忽略缓存
            ...
            var adData: Data? = JDLAdvertisingView.getAdDataFromLocal(url)
            if adData == nil {
                // 当缓存超过5个的时候，清空缓存
                if let filePath = JDLAdvertisingView.rootFilePath() {
                    if FileManager.default.subpaths(atPath: filePath)?.count ?? 0 > 5 {
                        let _ = JDLAdvertisingView.clealAllLocalCache()
                    }
                }
                // 下载并缓存
                adData = try? Data.init(contentsOf: URL.init(string: url)!)
                let _ = JDLAdvertisingView.saveDataToLocal(url, data: adData)
            }
            return adData
        }
    }
